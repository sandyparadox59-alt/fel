...
def is_owner(user_id):
    return user_id in (OWNER_ID if isinstance(OWNER_ID, list) else [OWNER_ID])

def is_reseller(user_id):
    return user_id in (RESELLERS if isinstance(RESELLERS, list) else [RESELLERS])
# ----------------------------
# init
# ----------------------------
...
def install_plugin(name: str, path: Path):
    ...
    try:
        ...
        if hasattr(module, "setup") and callable(module.setup):
            try:
                ret = module.setup(client, db, logger, is_owner=is_owner, is_reseller=is_reseller)
            except TypeError:
                # maybe plugin.setup is async? support both sync and async
                coro = module.setup(client, db, logger, is_owner=is_owner, is_reseller=is_reseller)
...
async def message_logger(event):
        ...
        text = event.text or ""
        logger.info(f"Pesan masuk dari {sender_name} ({sender.id}): {text}")
